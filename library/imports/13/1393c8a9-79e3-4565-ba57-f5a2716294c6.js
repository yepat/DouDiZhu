...
cc.Class({
    ...
    clearTable: function clearTable() {
        ...
        if (this.watingStart) this.watingStart.hideNode();
    ...
    autoLiftUpCard: function autoLiftUpCard() {
        //智能提牌测试
    ...
    getCardsLarger: function getCardsLarger() {
        //获取大于上家的牌
    ...
    onMyOutCard: function onMyOutCard(time, tishicards) {
        var self = this;
        ...
        //new
        var chupaiType = 0;
        if (this.playerme_.getCardCount() == 20 || this.mySeatId == this.preSeatId) {} else {
            if (tishicards && tishicards.length == 0) {
                curTime = 3;
                chupaiType = 2;
            }
    ...
    btnLeftPlayerClick: function btnLeftPlayerClick() {
        console.log("点击左边头像");
        var args = {};
    ...
    btnRightPlayerClick: function btnRightPlayerClick() {
        console.log("点击右边头像");
        var args = {};
    ...
    showJPQShareGet: function showJPQShareGet() {
        //记牌器开启
        var self = this;
    ...
    showMyCardtype: function showMyCardtype(cardType) {
    ...
    showLeftCardtype: function showLeftCardtype(cardType) {
        var self = this;
    ...
    showRightCardtype: function showRightCardtype(cardType) {
        var self = this;
        var numUrl = "showTips/p_cardtype_1";
        if (cardType == config.CardType.Straight) {} else if (cardType == config.CardType.StraightDouble) {
            numUrl = "showTips/p_cardtype_2";
    ...
    showWatingStartGame: function showWatingStartGame() {
        var self = this;
    ...
    onQueued: function onQueued(event) {
        ...
        if (self.gameState > 1 && self.gameState < 8) return;

        self.gameState = config.gameState.ST_GAME_PREPARE;
        if (self.watingStart) self.watingStart.showNode();
    },
    ...
    onSendCard: function onSendCard(event) {
        ...
        if (self.watingStart) self.watingStart.hideNode();
        ...
        var moveCardend = function moveCardend() {
    ...
    onPlayerCancelDelegate: function onPlayerCancelDelegate(event) {
        var self = this;
        console.log("用户取消托管---");
        var response = event.getUserData();
        var seatId = response["data"]["trustId"];
    ...
    onInvalidCardType: function onInvalidCardType(event) {
        var self = this;
    ...
    onReconnectionData: function onReconnectionData(event) {
        ...
        if (players.lordId != 4) {
            ...
            if (config.joker != "") self.setLazarillo_poker(config.joker);
    ...
    onEmoticonData: function onEmoticonData(event) {
        console.log("广播表情包------");
        var self = this;
        var response = event.getUserData();
        // console.log(data);
    ...
    showEmoticon: function showEmoticon(args) {
    ...
    onShareWxResResult: function onShareWxResResult(event) {
    ...
    showLazarilloPokerSelected: function showLazarilloPokerSelected(cardsList) {
        var self = this;
        if (self.LazarilloPokerSelected) return;
        var click = function click() {
            // self.LazarilloPokerSelected.closeClick();
            self.LazarilloPokerSelected = null;
        };
